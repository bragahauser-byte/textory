...
function EspacoLinhas({ onBack }) {
  ...
  const { t } = useLocale()
  return <main className="screen-shell min-h-screen bg-white text-[#202020]"><SettingsHeader title={t('settings.lineSpacing')} onBack={onBack} /><Slider className="mt-6" value={lineSpacing} min={0} max={2} step={1} onChange={(nextValue) => updateSetting('lineSpacing', nextValue)} /><div className="reading-preview preview-transition mt-6 h-[405px] overflow-hidden rounded-xl border border-[#eeeeee] p-3" style={{ fontFamily, fontSize: `${fontSize}px`, lineHeight: `${lineHeight}px`, letterSpacing }}>{t('preview')}</div></main>
}
