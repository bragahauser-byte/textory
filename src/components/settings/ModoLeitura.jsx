...
function ModoLeitura({ onBack }) {
  ...
  const { t } = useLocale()
  return <main className="bg-[var(--color-bg)] text-[var(--color-text)]"><SettingsHeader title={t('settings.readingMode')} onBack={onBack} /><div className="screen-header-gap">{modes.map((key) => <OptionRow key={key} selected={readingMode === key} onClick={() => updateSetting('readingMode', key)}>{t(`settings.modes.${key}`)}</OptionRow>)}</div><ReadingPreview key={readingMode} height={250} className="reading-preview content-enter title-subtitle-gap rounded-xl p-3" style={{ background: colors.background, color: colors.primary, fontFamily, fontSize: `${Math.min(fontSize, 24)}px`, lineHeight: `${lineHeight}px`, letterSpacing }}>{t('preview')}</ReadingPreview></main>
}
