...
function Configuracoes({ onBack, onOpen }) {
  ...
  return <main className="bg-white text-[#202020]"><BackButton onBack={onBack} /><div className="screen-header-gap"><SettingsRow label={t('settings.language')} value={t(`settings.languages.${language}`)} onClick={() => onOpen('idioma')} /><SettingsRow label={t('settings.textSize')} value={sizeLabel} onClick={() => onOpen('tamanho')} /><SettingsRow label={t('settings.textStyle')} value={styleLabel} onClick={() => onOpen('estilo')} /><SettingsRow label={t('settings.lineSpacing')} value={spacingLabel} onClick={() => onOpen('espaco')} /><SettingsRow label={t('settings.readingMode')} value={modeLabel} onClick={() => onOpen('modo')} /></div></main>
}
