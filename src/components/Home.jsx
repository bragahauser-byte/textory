        ...
        <ul className="mt-12 flex-1 space-y-3 overflow-y-auto pr-0">
          {items.map((item, index) => (
            <li key={item.id ?? item.title} className="list-item-enter" style={{ animationDelay: `${index * 35}ms` }}>
              <button type="button" onClick={() => onSelectReading?.(item)} className="flex min-h-[76px] w-full items-center gap-4 rounded-xl border border-[#F5F5F5] px-3 py-4 text-left transition-colors duration-120 hover:border-[#bdbdbd] active:scale-[0.98] focus:outline-none focus:ring-2 focus:ring-black">
                ...
                <span className="min-w-0 flex-1"><strong className="type-card-title block truncate font-bold text-[#1A1A1A]">{item.title}</strong><span className="type-small card-subtitle-gap block font-medium text-[#787878]">{item.duration} · {item.sections} {item.sections === 1 ? 'seção' : 'seções'}</span></span>
                <span className="flex size-11 shrink-0 items-center justify-center rounded-full text-[#777777]" aria-hidden="true"><ChevronRight size={24} /></span>
              </button>
            </li>
          ))}
        </ul>
