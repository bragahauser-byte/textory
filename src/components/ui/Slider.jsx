...
function Slider({ value, onChange, min = 0, max = 100, step = 1, steps, className = '' }) {
  ...
  return (
    ...
    >
      <span className="absolute left-0 right-0 top-1/2 h-2 -translate-y-1/2 rounded-full bg-[var(--slider-track-empty)]" style={{ background: isRTL ? `linear-gradient(to right, var(--slider-track-empty) ${visualPercentage}%, var(--slider-track-fill) ${visualPercentage}%)` : `linear-gradient(to right, var(--slider-track-fill) ${visualPercentage}%, var(--slider-track-empty) ${visualPercentage}%)` }} />
